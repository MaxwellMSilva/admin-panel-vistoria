  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData?.error || "Falha ao criar modelo")
      }

      toast.success("Modelo criado com sucesso", { duration: 2000 })
      onSuccess()
      reset()
    } catch (error: any) {
      toast.error(error.message || "Erro ao criar modelo", { duration: 2000 })
    }
  }
